...
async function carregarLista() {
  const { data, error } = await supabase.from('ListaDeCompras').select('*')
  if (error) {
...
window.adicionarItem = async function () {
  ...
  const { error } = await supabase.from('ListaDeCompras').insert({
    item: input.value,
...
window.removerItem = async function (id) {
  const { error } = await supabase.from('ListaDeCompras').delete().eq('id', id)
